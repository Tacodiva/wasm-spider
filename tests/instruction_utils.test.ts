import { SpiderConstExpression, WasmOpcode, WasmValueType, spider } from "../src";
...
describe("Expression", () => {
    test('emitConstant', async () => {
        ...
        expect(expr.instructions[0]).toEqual(expect.objectContaining({ opcode: WasmOpcode.f32_const, args: [-77.29] }));

        expr.emitConstant(WasmValueType.f64, -77.29);
        expect(expr.instructions[1]).toEqual(expect.objectContaining({ opcode: WasmOpcode.f64_const, args: [-77.29] }));

        expr.emitConstant(WasmValueType.i32, -7729);
        expect(expr.instructions[2]).toEqual(expect.objectContaining({ opcode: WasmOpcode.i32_const, args: [-7729] }));

        expr.emitConstant(WasmValueType.i64, -7729);
        expect(expr.instructions[3]).toEqual(expect.objectContaining({ opcode: WasmOpcode.i64_const, args: [-7729] }));
    });
